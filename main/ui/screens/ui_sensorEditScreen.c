...
/* Sensores y alarmas — edición (mockup 5b). */
...
/* tarjeta base con caption arriba */
...
/* control segmentado: fila de opciones, la activa resaltada en verde */
static void seg_click_cb(lv_event_t *e)
{
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        lv_obj_set_style_text_color(lbl, ui_col(act ? 0x06251f : UI_C_TEXT_2), 0);
    }
}

static void seg_control(lv_obj_t *parent, const char *const *opts, int n, int active)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        lv_obj_t *l = ui_label(o, opts[i], UI_FONT_SM, act ? 0x06251f : UI_C_TEXT_2);
        lv_obj_center(l);
    }
}

static lv_obj_t *thr_box(lv_obj_t *parent, const char *label, const char *value, const char *unit)
{
    ...
    lv_obj_add_event_cb(b, ui_open_keypad_cb, LV_EVENT_CLICKED, NULL);
    ...
    ui_label(vw, value, UI_FONT_LG, UI_C_ALARM);
...
void ui_sensorEditScreen_screen_init(void)
{
    ...
    ui_label(gas, "Oxígeno · O₂", UI_FONT_LG, UI_C_OK);
    ...
    thr_box(thr_row, "Mínimo", "500", "psi");
    thr_box(thr_row, "Máximo", "2000", "psi");
